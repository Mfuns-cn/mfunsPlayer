...
const templateWrap = ({ label, template }: { label?: string; template: SwitchItemTemplate }) => html` <div class="${classPrefix}-switch">${template(label)}</div> `
...
export class Switch implements SwitchOptions {
  ...
  constructor({
    container, value = false, onChange, onToggle, template,
  }: SwitchOptions) {
