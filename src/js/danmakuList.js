...
class DanmakuList {
  constructor(player, el, options = {}) {
    ...
    this.autoScroll = false   // 弹幕列表自动滚动
    this.init();
  }
  init() {
  ...
  sort(sortedBy = this.sortedBy, reverse = this.sortReverse) {  // 弹幕列表排序
    this.sortedBy = sortedBy
    this.sortReverse = reverse
    this.listData.sort((a, b) => {  // 排序
  ...
  reload() {    // 重载弹幕列表
    ...
    this.listData = this.data.filter((item) => {
      return !(
        ...
        (specialFilter && item.mode > 6)      // 特殊弹幕屏蔽
      )
    })
    this.listData.sort((a, b) => {  // 排序
      return this.sortReverse ? b[this.sortedBy] - a[this.sortedBy] : a[this.sortedBy] - b[this.sortedBy]
