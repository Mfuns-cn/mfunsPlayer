...
export const pictureInPictureEnabled: boolean = document.pictureInPictureEnabled
...
export const textToSecond = (str: string): number => {
  ...
  let sec = parseInt(arr[arr.length - 1]) || 0;
  let min = parseInt(arr[arr.length - 2]) || 0;
  let hour = parseInt(arr[arr.length - 3]) || 0;
...
export const number2Color = (number: number) =>
