...
class Danmaku {
    ...
    initDanmakuTips() {
        const danmakuTipMaskMove = (e) => {
            if (Math.sqrt(Math.pow(e.pageX - this.currentX, 2) + Math.pow(e.pageY - this.currentY, 2)) > this.player.options.danmaku.sensitivity ?? 3.5) {
            ...
            allReports.forEach((el, index) => {
                if (!this[`reportOption${index}`]) {
                    this[`reportOption${index}`] = new Switch(el, false, {
                        ...
                        off: (optionTrigger) => {
                            // 关闭开关
                            if (this.player.template.danmakuReportDetail.classList.contains('show')) this.player.template.danmakuReportDetail.classList.remove('show');
                            if (!this.player.template.danmakuReportSubmit.classList.contains('disable') && !optionTrigger) this.player.template.danmakuReportSubmit.classList.add('disable');
                        },
                    });
                } else {
    ...
    load() {
        const { address, danmakuAddition, id, advDanApi, otherDanParams } = JSON.parse(JSON.stringify(this.options.api));
    ...
    reloadEndPoints(origin) {
        ...
        this._readAllEndpoints(currentEndPoints, (results, loadStatus) => {
            if (!loadStatus) {
                ...
                // const
                this.dan = [...(acfun ?? []), ...(bili ?? []), ...(mfuns ?? []), ...(unknown ?? [])].sort((a, b) => a.time - b.time);
                this.dan.forEach((d) => {
                    d.id = this.createHash(8);
                    if (d.type == 8) {
                        advDanData.push(d.text); // 筛选高级弹幕
    ...
    send(dan) {
        ...
        this.options.sendDanmaku &&
            this.options.sendDanmaku([+this.options.time().toFixed(2), utils.type2Number(dan.type ?? 'right'), utils.color2Number(dan.color ?? '#FFFFFF'), dan.text, +dan.size ?? 25], this.options.videoIndex());
    ...
    frame() {
        if (this.dan.length && !this.paused && this.showing) {
            ...
            if (this.miniMode) {
                dan = dan.map((el) => Object.assign(el, { size: 16 }));
            }
            if (dan.length) {
                // console.time(this.danIndex);
                this.draw(dan, this.drawIndex + 1);
                // console.timeEnd(this.danIndex);
                this.drawIndex++;
            }
        }
        window.requestAnimationFrame(() => {
    ...
    //弹幕绘制
    draw(dan, index) {
        // console.log(dan.length, `第${index + 1}次渲染`);
        if (this.showing) {
            ...
            const danItemLeft = (ele) => {
                ...
                return eleLeft - this.container.getBoundingClientRect().left;
            };
            //弹幕速度
            const danSpeed = (width) => ((danWidth + width) / this.player.fullScreen.isFullScreen() ? 8 : 5) * (this._speedVal ?? 1);
            const realDOMTunnel = (type, i) => {
                // if (type !== "right" && type !== "left") {
                //   return;
                // }
                return [...this.container.querySelectorAll(`.mfunsPlayer-danmaku-${type}`)].filter((el) => el.dataset.tunnel === `${i}`);
            };
            //获取弹幕可进入的轨道
            let tunnelPush = {
                right: {},
                ...
                for (let i = 0; this.unlimited || i < itemY; i++) {
                    ...
                    if (item && item.length) {
                        ...
                        for (let j = 0; j < item.length; j++) {
                            ...
                            if (j === item.length - 1) {
                                // console.log(this.danTunnel[type]);
                                //轨道弹幕组遍历完毕，组内所有弹幕均完全进入容器，可以向该轨道装填弹幕
                                if (tunnelPush[type] && tunnelPush[type][i + ''] && !this.unlimited) {
                                    break;
                                }
                                this.danTunnel[type][i + ''].push(ele);
                                tunnelPush[type][i + ''] = true;
            ...
            for (let i = 0; i < dan.length; i++) {
                ...
                item.addEventListener('animationend', () => {
                    [...this.container.children].includes(item) && this.container.removeChild(item);
                });

                const itemWidth = this._measure(dan[i].text, +dan[i].size ?? 25);

                let tunnel, realTunnel;
                // adjust
                switch (dan[i].type) {
                    case 'right':
                        ...
                        if (tunnel >= 0 || dan[i].isSubtitle) {
                            ...
                            if (top + parseInt(item.style.fontSize) + this.tunnelPadding > danHeight) {
                                this.danTunnel[dan[i].type][tunnel]?.pop();
                                continue;
                            }
    ...
    _measure(text, size) {
        ...
        this.context.font = measureStyle.getPropertyValue('font') ?? `${fontWeight}  ${fontSize} / ${fontSize} ${fontFamily}`;
    ...
    mini(flag) {
        this.miniMode = flag;
        this.options.height = flag ? 22 : 31;
    }
}
