...
class Danmaku {
    ...
    load() {
        ...
        const advDanData = [];
        id && address && this.endpoints.push({ url: apiurl, type: 'mfuns-danmaku', origin: 'mfuns' });
        advDanApi &&
            this.endpoints.push({
        ...
        this._readAllEndpoints(this.endpoints, (results, loadStatus) => {
            ...
            if (!loadStatus) {
                ...
                console.log('[mfuns-player] 弹幕加载结果', results);
                ...
                console.log(`[mfuns-player] 弹幕加载数量${JSON.stringify(this.danCount)}`);
        ...
        if (this.showing) {
            ...
            const getTunnel = (ele, type, width) => {
                ...
                for (let i = 0; this.unlimited || i < itemY; i++) {
                    ...
                    //   break;
                    // }
                    if (itemY < i) console.log('[mfuns-player] -------', i);
                    if (item && item.length) {
                        if (type !== 'right' && type !== 'left') {
                            continue;
                        }
                        ...
                        for (let j = 0; j < item.length; j++) {
                            if (type === 'right') {
                                ...
                                if (danRight <= 0) {
                                    this.tunnelHeights[type][i] = parseInt(this.miniMode ? 16 : item[j].style.fontSize) + this.tunnelPadding;
                    ...
                    // console.log("[mfuns-player] hhhhhhh", this.danIndex % itemY, type);
    ...
    clear(type) {
        console.log('[mfuns-player] clear');
        if (type) {
            this.danTunnel[type] = {};
            return;
        }
