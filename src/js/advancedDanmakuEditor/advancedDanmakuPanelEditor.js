...
class AdvancedDanmakuPanelEditor {
    ...
    init() {
        ...
        this.view.ade_panel_preview.onclick = () => {
            this.preview();
        };
        this.view.ade_panel_emit.onclick = () => {
            this.send();
        };
        this.view.ade_panel_copy.onclick = () => {
            navigator.clipboard.writeText(this.generateDanmaku() || '').then((res) => {
        ...
        this.c.action_getCurrentTime.onclick = () => {
            this.getCurrentTime();
        };
        this.c.action_delKeyFrame.onclick = () => {
            this.delKeyFrame();
        };
        this.c.action_addKeyFrame.onclick = () => {
            this.addKeyFrame();
        };

        this.c.switch_shadow.onchange = (e) => {
            this.c.area_content.classList.toggle('ad-use-shadow', e.currentTarget.checked);
        };
        this.c.switch_stroke.onchange = (e) => {
            this.c.area_content.classList.toggle('ad-use-stroke', e.currentTarget.checked);
        };

        this.c.switch_position.onchange = (e) => {
            this.c.area_animation_settings.classList.toggle('ad-use-position', e.currentTarget.checked);
        };
        this.c.switch_rotation.onchange = (e) => {
            this.c.area_animation_settings.classList.toggle('ad-use-rotation', e.currentTarget.checked);
        };
        this.c.switch_scale.onchange = (e) => {
            this.c.area_animation_settings.classList.toggle('ad-use-scale', e.currentTarget.checked);
        };
        this.c.switch_opacity.onchange = (e) => {
            this.c.area_animation_settings.classList.toggle('ad-use-opacity', e.currentTarget.checked);
        };
    }
    addKeyFrame() {
        // 在第n关键帧后添加新关键帧
    ...
    delKeyFrame() {
        ...
        if (this.keyFrames.length < 2) {
            console.log('[mfuns-player] 不能删除初始状态');
        } else {
            console.log(n);
    ...
    useAutoCurrentTime(b) {
        // 自动使用当前时间
        this.autoCurrentTime = b;
    }
    getDanmakuData() {
        // 生成弹幕, 并对部分未设置的值进行属性删除处理，以减小弹幕字符串大小
        this.saveKeyFrame();
        let d = {
