...
const template = (sort: { time: () => void; content: () => void; date: () => void }) => html`
  <div class="${classPrefix}-side-panel ${classPrefix}-danmaku-list">
    ...
    <div class="${classPrefix}-danmaku-list-foot">
      <div class="${classPrefix}-danmaku-list-foot-left">
        <span class="${classPrefix}-danmaku-list-autoscroll">列表滚动[关]</span>
      </div>
      <div class="${classPrefix}-danmaku-list-foot-right"></div>
    </div>
  </div>
`;
...
export default class DanmakuList {
  ...
  constructor(player: Player) {
    ...
    this.$autoScroll = fragment.querySelector(`.${classPrefix}-danmaku-list-autoscroll`)!;
    ...
    this.player.on("change:danmaku_list_auto_scroll", (flag) => {
      if (flag) {
    ...
    if (this.autoScroll) {
      this.player.events.trigger("change:danmaku_list_auto_scroll", true);
    }
  }
  init() {
  ...
  setAutoScroll(flag: boolean) {
    this.player.events.trigger("change:danmaku_list_auto_scroll", flag);
    // 设置自动滚动
