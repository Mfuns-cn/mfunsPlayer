class Ae {
  ...
  register(t, e) {
    ...
    (n = s.created) == null || n.call(s, e), this.initialized && ((l = s.pluginsReady) == null || l.call(s, e), (r = s.playerMounted) == null || r.call(s, e), (o = s.videoInited) == null || o.call(s, e));
  }
  /** 批量注册插件 */
  pluginsRegister(t) {
    var e;
...
class Pe {
  ...
  emit(t, ...e) {
    ...
    if ((s = this.listeners[t]) != null && s.length)
      for (let l = 0; l < this.listeners[t].length; l++)
        this.listeners[t][l](...e);
    if ((n = this.onceListeners[t]) != null && n.length) {
      for (let l = 0; l < this.onceListeners[t].length; l++)
        this.onceListeners[t][l](...e);
...
const nt = /mobile/i.test(window.navigator.userAgent), W = (i, t) => Array.from({ length: i }, () => t), Lt = () => document.fullscreenElement || document.mozFullScreenElement || document.webkitFullscreenElement || document.msFullscreenElement || null, _t = document.fullscreenEnabled || document.webkitFullscreenEnabled || document.mozFullScreenEnabled || document.msFullscreenEnabled || !1, Et = document.pictureInPictureEnabled, ht = (i) => {
...
const De = (i, t) => {
  ...
  get: (i) => localStorage.getItem(i)
}, qe = {
  dragStart: nt ? "touchstart" : "mousedown",
  dragMove: nt ? "touchmove" : "mousemove",
  dragEnd: nt ? "touchend" : "mouseup"
}, Me = (i, t) => {
  i = String(i).replace(/[^0-9a-f]/gi, ""), i.length < 6 && (i = i[0] + i[0] + i[1] + i[1] + i[2] + i[2]), t = t || 0;
  let e = "#", s, n;
  for (n = 0; n < 3; n++)
    s = parseInt(i.substr(n * 2, 2), 16), s = Math.round(Math.min(Math.max(0, s + s * t), 255)).toString(16), e += `00${s}`.substr(s.length);
  return e;
}, Ne = (i, t) => {
  if (!/^#([0-9A-Fa-f]{3}|[0-9A-Fa-f]{6})$/.test(i))
    return;
  let s = i.toLowerCase().replace(/#/g, "");
  const n = s.length;
  if (n == 3) {
    let r = "";
    for (let o = 0; o < n; o++)
      r += s.slice(o, o + 1).concat(s.slice(o, o + 1));
    s = r;
  }
  const l = [];
  for (let r = 0; r < 6; r += 2) {
    const o = s.slice(r, r + 2);
    l.push(parseInt(`0x${o}`));
  }
  return t ? `rgba(${l.join(",")},${t})` : `rgb(${l.join(",")})`;
}, Ie = (i) => {
  if (!/^(rgb|RGB)/.test(i))
    return;
  const e = i.slice(4, i.length - 1).split(",");
  let s = "#";
  for (let n = 0; n < e.length; n++) {
    let l = Number(e[n]).toString(16);
    l == "0" && (l += "0"), s += l;
  }
  return s;
}, Tt = (i) => (i[0] === "#" && (i = i.substr(1)), i.length === 3 && (i = `${i[0]}${i[0]}${i[1]}${i[1]}${i[2]}${i[2]}`), parseInt(i, 16) + 0 & 16777215), At = (i) => `#${`00000${i.toString(16)}`.slice(-6)}`, Be = () => {
  let i = 100;
  return function(t) {
    if (!t || typeof Number(t) != "number")
      return;
    const e = [
      "1",
      "2",
      ...
      "y",
      "z"
    ], s = [], n = Number(t), l = e.length;
    for (let r = 0; r < n; r++)
      s.push(e[Math.floor(Math.random() * l)]);
    return i++, `${s.join("")}${i}`;
  };
}, Re = (i) => {
  const t = [16, 18, 25, 36, 45, 64], e = Math.floor(Math.random() * i);
  return t[e];
}, at = {
  ...
  dateFormat: ut,
  debounce: dt,
  fullScreenEnabled: _t,
  getFullscreenElement: Lt,
  hex2Rgb: Ne,
  initHash: Be,
  nameMap: qe,
  number2Color: At,
  pictureInPictureEnabled: Et,
  randomFontsize: Re,
  rgb2Hex: Ie,
  secondToTime: V,
  storage: He,
  throttle: De,
  timeToSecond: ht
}, Symbol.toStringTag, { value: "Module" })), a = "mfuns-player", Oe = "https://github.com/Mfuns-cn/mfunsPlayer/tree/v3-beta", ze = [
...
class We {
  constructor(t, e) {
    this.ratio = null, this.player = t, this.$video = this.player.$content.appendChild(
      P("video", { class: `${a}-video` })
    ), this.loader = new Fe(this), this.setRatio(e.aspectRatio || null), this.attachEvent(this.$video), this.initKeepRatio(), this.player.on("play", () => {
  ...
  setRatio(t) {
    if (this.ratio = t, this.$video.style.width = "", this.$video.style.height = "", t) {
      ...
      const { width: n, height: l } = this.$video.getBoundingClientRect(), { width: r, height: o } = this.player.$area.getBoundingClientRect();
      n == r && l == o && this.rescale(n, l, e, s);
    } else
  ...
  /** 保持视频比例 */
  initKeepRatio() {
    this.player.on("video_resize", ([t, e]) => {
      if (this.ratio) {
...
class Ue {
  constructor(t, e) {
    this.focused = !1, this.active = !1, this.controlled = !1, this.intersecting = !0, this.mousemove = !1, this.player = t, this.activeTime = e.activeTime ?? 3e3, window.ResizeObserver && (this.resizeObserver = new ResizeObserver(([s]) => {
      const { width: n, height: l } = s.contentRect;
      this.player.emit("resize", [n, l]);
    })), window.IntersectionObserver && (this.intersectionObserver = new window.IntersectionObserver(([s]) => {
      const { isIntersecting: n } = s;
      this.intersecting = n, this.player.emit("intersection", n);
    })), this.player.once("mounted", () => {
      var s, n;
      (s = this.resizeObserver) == null || s.observe(this.player.$el), (n = this.intersectionObserver) == null || n.observe(this.player.$el);
    }), this.init();
  }
  focus() {
  ...
  async call(t, e, s = !0) {
    ...
    if (n != null && n.length)
      for (const l of n) {
        const r = await l(e);
        if (r == !0)
          return console.log(`钩子提前结束调用: ${t}`), console.log(l), !0;
        if (r == !1)
          return console.log(`钩子被拦截: ${t}`), console.log(l), !1;
      }
    return console.log(`钩子调用完毕: ${t}`), console.log(e), s;
  }
}
/**
 * @license
 ...
 */
var lt;
const et = window, j = et.trustedTypes, $t = j ? j.createPolicy("lit-html", { createHTML: (i) => i }) : void 0, ct = "$lit$", N = `lit$${(Math.random() + "").slice(9)}$`, Pt = "?" + N, Xe = `<${Pt}>`, O = document, Q = () => O.createComment(""), Z = (i) => i === null || typeof i != "object" && typeof i != "function", Ft = Array.isArray, Ye = (i) => Ft(i) || typeof (i == null ? void 0 : i[Symbol.iterator]) == "function", rt = `[ 	
\f\r]`, Y = /<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g, gt = /-->/g, vt = />/g, I = RegExp(`>|${rt}(?:([^\\s"'>=/]+)(${rt}*=${rt}*(?:[^ 	
\f\r"'\`<>=]|("|')|))|$)`, "g"), ft = /'/g, yt = /"/g, Dt = /^(?:script|style|textarea|title)$/i, Ke = (i) => (t, ...e) => ({ _$litType$: i, strings: t, values: e }), u = Ke(1), G = Symbol.for("lit-noChange"), F = Symbol.for("lit-nothing"), bt = /* @__PURE__ */ new WeakMap(), R = O.createTreeWalker(O, 129, null, !1);
function Ht(i, t) {
...
const Qe = (i, t) => {
  ...
  for (let o = 0; o < e; o++) {
    const h = i[o];
    let c, p, v = -1, y = 0;
    for (; y < h.length && (r.lastIndex = y, p = r.exec(h), p !== null); )
      y = r.lastIndex, r === Y ? p[1] === "!--" ? r = gt : p[1] !== void 0 ? r = vt : p[2] !== void 0 ? (Dt.test(p[2]) && (n = RegExp("</" + p[2], "g")), r = I) : p[3] !== void 0 && (r = I) : r === I ? p[0] === ">" ? (r = n ?? Y, v = -1) : p[1] === void 0 ? v = -2 : (v = r.lastIndex - p[2].length, c = p[1], r = p[3] === void 0 ? I : p[3] === '"' ? yt : ft) : r === yt || r === ft ? r = I : r === gt || r === vt ? r = Y : (r = I, n = void 0);
    const E = r === I && i[o + 1].startsWith("/>") ? " " : "";
    l += r === Y ? h + Xe : v >= 0 ? (s.push(c), h.slice(0, v) + ct + h.slice(v) + N + E) : h + N + (v === -2 ? (s.push(void 0), o) : E);
  }
  return [Ht(i, l + (i[e] || "<?>") + (t === 2 ? "</svg>" : "")), s];
};
class J {
  constructor({ strings: t, _$litType$: e }, s) {
    ...
    this.parts = [];
    let l = 0, r = 0;
    const o = t.length - 1, h = this.parts, [c, p] = Qe(t, e);
    if (this.el = J.createElement(c, s), R.currentNode = this.el.content, e === 2) {
      const v = this.el.content, y = v.firstChild;
      y.remove(), v.append(...y.childNodes);
    }
    for (; (n = R.nextNode()) !== null && h.length < o; ) {
      if (n.nodeType === 1) {
        if (n.hasAttributes()) {
          const v = [];
          for (const y of n.getAttributeNames())
            if (y.endsWith(ct) || y.startsWith(N)) {
              const E = p[r++];
              if (v.push(y), E !== void 0) {
                const _ = n.getAttribute(E.toLowerCase() + ct).split(N), m = /([.?@])?(.*)/.exec(E);
                h.push({ type: 1, index: l, name: m[2], strings: _, ctor: m[1] === "." ? Ge : m[1] === "?" ? Ce : m[1] === "@" ? ts : st });
              } else
                h.push({ type: 6, index: l });
            }
          for (const y of v)
            n.removeAttribute(y);
        }
        if (Dt.test(n.tagName)) {
          const v = n.textContent.split(N), y = v.length - 1;
          if (y > 0) {
            n.textContent = j ? j.emptyScript : "";
            for (let E = 0; E < y; E++)
              n.append(v[E], Q()), R.nextNode(), h.push({ type: 2, index: ++l });
            n.append(v[y], Q());
          }
        }
        ...
        if (n.data === Pt)
          ...
          let v = -1;
          for (; (v = n.data.indexOf(N, v + 1)) !== -1; )
            h.push({ type: 7, index: l }), v += N.length - 1;
        }
      l++;
    }
  }
...
function X(i, t, e = i, s) {
  ...
  return (h == null ? void 0 : h.constructor) !== c && ((l = h == null ? void 0 : h._$AO) === null || l === void 0 || l.call(h, !1), c === void 0 ? h = void 0 : (h = new c(i), h._$AT(i, e, s)), s !== void 0 ? ((r = (o = e)._$Co) !== null && r !== void 0 ? r : o._$Co = [])[s] = h : e._$Cl = h), h !== void 0 && (t = X(i, h._$AS(i, t.values), h, s)), t;
}
class Ze {
  constructor(t, e) {
  ...
  u(t) {
    var e;
    const { el: { content: s }, parts: n } = this._$AD, l = ((e = t == null ? void 0 : t.creationScope) !== null && e !== void 0 ? e : O).importNode(s, !0);
    R.currentNode = l;
    let r = R.nextNode(), o = 0, h = 0, c = n[0];
    for (; c !== void 0; ) {
      if (o === c.index) {
        let p;
        c.type === 2 ? p = new C(r, r.nextSibling, this, t) : c.type === 1 ? p = new c.ctor(r, c.name, c.strings, this, t) : c.type === 6 && (p = new es(r, this, t)), this._$AV.push(p), c = n[++h];
      }
      o !== (c == null ? void 0 : c.index) && (r = R.nextNode(), o++);
    }
    return R.currentNode = O, l;
  }
  v(t) {
    let e = 0;
...
class C {
  constructor(t, e, s, n) {
    var l;
    this.type = 2, this._$AH = F, this._$AN = void 0, this._$AA = t, this._$AB = e, this._$AM = s, this.options = n, this._$Cp = (l = n == null ? void 0 : n.isConnected) === null || l === void 0 || l;
  }
  get _$AU() {
    var t, e;
  ...
  _$AI(t, e = this) {
    t = X(this, t, e), Z(t) ? t === F || t == null || t === "" ? (this._$AH !== F && this._$AR(), this._$AH = F) : t !== this._$AH && t !== G && this._(t) : t._$litType$ !== void 0 ? this.g(t) : t.nodeType !== void 0 ? this.$(t) : Ye(t) ? this.T(t) : this._(t);
  }
  k(t) {
  ...
  _(t) {
    this._$AH !== F && Z(this._$AH) ? this._$AA.nextSibling.data = t : this.$(O.createTextNode(t)), this._$AH = t;
  }
  g(t) {
    var e;
    const { values: s, _$litType$: n } = t, l = typeof n == "number" ? this._$AC(t) : (n.el === void 0 && (n.el = J.createElement(Ht(n.h, n.h[0]), this.options)), n);
    if (((e = this._$AH) === null || e === void 0 ? void 0 : e._$AD) === l)
      this._$AH.v(s);
    else {
      const r = new Ze(l, this), o = r.u(this.options);
  ...
  T(t) {
    ...
    let s, n = 0;
    for (const l of t)
      n === e.length ? e.push(s = new C(this.k(Q()), this.k(Q()), this, this.options)) : s = e[n], s._$AI(l), n++;
...
class st {
  constructor(t, e, s, n, l) {
    this.type = 1, this._$AH = F, this._$AN = void 0, this.element = t, this.name = e, this._$AM = n, this.options = l, s.length > 2 || s[0] !== "" || s[1] !== "" ? (this._$AH = Array(s.length - 1).fill(new String()), this.strings = s) : this._$AH = F;
  }
  get tagName() {
  ...
  _$AI(t, e = this, s, n) {
    ...
    let r = !1;
    if (l === void 0)
      t = X(this, t, e, 0), r = !Z(t) || t !== this._$AH && t !== G, r && (this._$AH = t);
    else {
      const o = t;
      let h, c;
      for (t = l[0], h = 0; h < l.length - 1; h++)
        c = X(this, o[s + h], e, h), c === G && (c = this._$AH[h]), r || (r = !Z(c) || c !== this._$AH[h]), c === F ? t = F : t !== F && (t += (c ?? "") + l[h + 1]), this._$AH[h] = c;
    }
    r && !n && this.j(t);
  }
  j(t) {
...
const Je = j ? j.emptyScript : "";
class Ce extends st {
  constructor() {
    super(...arguments), this.type = 4;
  }
  j(t) {
    t && t !== F ? this.element.setAttribute(this.name, Je) : this.element.removeAttribute(this.name);
  }
}
class ts extends st {
  constructor(t, e, s, n, l) {
  ...
  _$AI(t, e = this) {
    var s;
    if ((t = (s = X(this, t, e, 0)) !== null && s !== void 0 ? s : F) === G)
      return;
    const n = this._$AH, l = t === F && n !== F || t.capture !== n.capture || t.once !== n.once || t.passive !== n.passive, r = t !== F && (n === F || l);
...
const kt = et.litHtmlPolyfillSupport;
kt == null || kt(J, C), ((lt = et.litHtmlVersions) !== null && lt !== void 0 ? lt : et.litHtmlVersions = []).push("2.8.0");
const g = (i, t, e) => {
  var s, n;
  ...
  if (r === void 0) {
    ...
    l._$litPart$ = r = new C(t.insertBefore(Q(), o), o, void 0, e ?? {});
  }
  return r._$AI(i), r;
}, ss = ({ divider: i }) => u` <div
    ...
    <div class="mpui-slider-thumb-track" style="height: 0px">
      ...
      ${i ? u`
            <div class="mpui-slider-divider">
              ${W(i, u`<div class="mpui-slider-divider-dot"></div>`)}
            </div>
          ` : ""}
    </div>
  </div>
...
class K {
  constructor({
    ...
    this.container = t, this.min = e, this.max = s, this.step = n || 0, this.divider = l ? typeof l == "boolean" ? this.step : l : 0, this.value = isNaN(r) ? r : Number(r), this.onChange = o, this.onDragStart = h, this.onDragEnd = c, this.onDrag = p, g(ss({ divider: this.divider }), t), this.$el = this.container.querySelector(".mpui-slider"), this.$track = this.$el.querySelector(".mpui-slider-track"), this.$bar = this.$track.querySelector(".mpui-slider-bar"), this.$thumbTrack = this.$track.querySelector(".mpui-slider-thumb-track"), this.$thumb = this.$track.querySelector(".mpui-slider-thumb"), this.$el.addEventListener("mousedown", (v) => {
      var w;
      const y = v, { clientX: E } = y, _ = this.$track.offsetWidth;
      let m = this.$thumbTrack.offsetWidth;
      m = m || _;
      const $ = (_ - m) / 2, d = this.$el.getBoundingClientRect().left;
      let f = E - d - $;
      f = f >= m ? m : f <= 0 ? 0 : f;
      const L = this.step ? Math.round(f / m * (this.max - this.min) / this.step) * this.step + this.min : f / m * (this.max - this.min) + this.min;
      (w = this.onDragStart) == null || w.call(this, L), this.value != L && this.drag(L);
      const k = (x) => {
        var pt;
        const D = x, { clientX: b } = D;
        D.preventDefault(), D.stopPropagation();
        let T = b - d - $;
        T = T >= m ? m : T <= 0 ? 0 : T;
        const q = this.step ? Math.round(T / m * (this.max - this.min) / this.step) * this.step + this.min : T / m * (this.max - this.min) + this.min;
        this.value != q && this.drag(q), (pt = window.getSelection()) == null || pt.removeAllRanges();
      }, A = (x) => {
        var b, T;
        x.stopPropagation(), (b = window.getSelection()) == null || b.removeAllRanges(), document.removeEventListener("mousemove", k), document.removeEventListener("mouseup", A), (T = this.onDragEnd) == null || T.call(this, L);
      };
...
class qt {
  constructor({
    ...
    this.container = t, this.min = e, this.max = s, this.step = n || 0, this.value = isNaN(l) ? l : Number(l), this.onChange = r, this.onDragStart = o, this.onDragEnd = h, this.onDrag = c, g(is(), t), this.$el = this.container.querySelector(".mpui-slider"), this.$track = this.$el.querySelector(".mpui-slider-track"), this.$bar = this.$track.querySelector(".mpui-slider-bar"), this.$thumbTrack = this.$track.querySelector(".mpui-slider-thumb-track"), this.$thumb = this.$track.querySelector(".mpui-slider-thumb"), this.$el.addEventListener("mousedown", (p) => {
      var A;
      const v = p, { clientY: y } = v, E = this.$track.offsetHeight;
      let _ = this.$thumbTrack.offsetHeight;
      _ = _ || E;
      const m = (E - _) / 2, $ = this.$el.getBoundingClientRect().top;
      let d = _ - (y - $ - m);
      d = d >= _ ? _ : d <= 0 ? 0 : d;
      const f = this.step ? Math.round(d / _ * (this.max - this.min) / this.step) * this.step + this.min : d / _ * (this.max - this.min) + this.min;
      (A = this.onDragStart) == null || A.call(this, f), this.value != f && this.drag(f);
      const L = (w) => {
        var q;
        const x = w, { clientY: D } = x;
        x.preventDefault(), x.stopPropagation();
        let b = _ - (D - $ - m);
        b = b >= _ ? _ : b <= 0 ? 0 : b;
        const T = this.step ? Math.round(b / _ * (this.max - this.min) / this.step) * this.step + this.min : b / _ * (this.max - this.min) + this.min;
        this.value != T && this.drag(T), (q = window.getSelection()) == null || q.removeAllRanges();
      }, k = (w) => {
        var D, b;
        w.stopPropagation(), (D = window.getSelection()) == null || D.removeAllRanges(), document.removeEventListener("mousemove", L), document.removeEventListener("mouseup", k), (b = this.onDragEnd) == null || b.call(this, f);
      };
...
`;
class U {
  constructor({ container: t, value: e, onChange: s, onPick: n, list: l, template: r, condition: o }) {
    this.container = t, this.list = l, this.value = e ?? null, this.onChange = s, this.onPick = n, this.template = r, this.condition = o, this.reload();
  }
  /** 重载，一般用于列表项更改 */
  reload(t) {
    g(ns({ list: this.list, template: this.template }), this.container), this.$el = this.container.querySelector(".mpui-picker"), this.$items = this.$el.querySelectorAll(".mpui-picker-item"), this.$items.forEach((e) => {
  ...
  setValue(t) {
    ...
    this.$items.forEach((s, n) => {
      (this.condition ? this.condition(s.getAttribute("data-value"), t) : s.getAttribute("data-value") == t) ? s.classList.add("state-checked") : s.classList.remove("state-checked");
...
class Mt {
  ...
  /** 重载，一般用于列表项更改 */
  reload(t) {
    g(as({ list: this.list, template: this.template }), this.container), this.$el = this.container.querySelector(".mpui-picker"), this.$items = this.$el.querySelectorAll(".mpui-picker-item"), this.$items.forEach((e) => {
      e.addEventListener("click", () => {
  ...
  setValue(t) {
    ...
    this.valueSet = new Set(t), this.$items.forEach((s, n) => {
      this.valueSet.has(s.getAttribute("data-value")) ? s.classList.add("state-checked") : s.classList.remove("state-checked");
  ...
  toggle(t, e) {
    ...
    s ? this.valueSet.add(t) : this.valueSet.delete(t), this.$items.forEach((r, o) => {
      r.getAttribute("data-value") == t && r.classList.toggle("state-checked", s);
    }), (n = this.onChange) == null || n.call(this, this.value), (l = this.onToggle) == null || l.call(this, t, s);
  }
}
const ls = ({ label: i }) => u` <div class="mpui-switch">${i}</div> `;
class rs {
  constructor({ container: t, value: e = !1, onChange: s, onToggle: n }) {
    this.container = t, this.value = e, this.onChange = s, this.onToggle = n, g(ls({ label: this.label }), this.container), this.$el = this.container.querySelector(".mpui-switch"), this.$el.addEventListener("click", () => {
...
class z {
  constructor({ container: t, value: e = !1, onChange: s, onToggle: n, label: l }) {
    this.container = t, this.value = e, this.onChange = s, this.onToggle = n, this.label = l, g(os({ label: this.label }), this.container), this.$el = this.container.querySelector(".mpui-checkbox"), this.$el.addEventListener("click", () => {
      this.toggle();
    }), this.setValue(this.value);
  }
  /** 设置开关状态 */
...
class M {
  constructor(t) {
    this.event = new Pe(), this.hook = new je(), this.plugin = {}, this.videoInfo = {}, this.container = t.container, this.$el = P("div", { class: `${a} mpui` }), this.$main = this.$el.appendChild(P("div", { class: `${a}-main` })), this.$area = this.$main.appendChild(P("div", { class: `${a}-area` })), this.$content = this.$area.appendChild(
      P("div", { class: `${a}-content` })
    ), this.pluginManager = new Ae(this), this.video = new We(this, t), this.state = new Ue(this, t), this.pluginManager.pluginsRegister(t), this.pluginManager.pluginsReady(t), this.container.appendChild(this.$el), this.pluginManager.playerMounted(t), this.emit("mounted"), this.setVideo(t.video, t.autoPlay, t.time);
  }
  get $video() {
...
console.log(
  `
 %c mfunsPlayer v3.0.0-dev 7469af9 %c https://github.com/Mfuns-cn 
...
class cs {
  ...
  render(t, e) {
    ...
    if (this.renderStart = this.getViewStart(e) - this.overflow, this.renderEnd = this.getViewEnd(e, t) + this.overflow, this.renderStart < s) {
      const l = document.createDocumentFragment(), r = Math.max(this.renderStart, 0), o = Math.min(s - 1, this.renderEnd, this.data.length - 1);
      for (let h = r; h <= o; h++)
        l.appendChild(this.createItem(this.data[h], h, this.data));
    ...
    if (this.renderEnd > n) {
      const l = document.createDocumentFragment(), r = Math.max(n + 1, this.renderStart), o = Math.min(this.renderEnd, this.data.length - 1);
      for (let h = r; h <= o; h++)
        l.appendChild(this.createItem(this.data[h], h, this.data));
...
class S {
  constructor(t) {
    this.player = t, this.plugin = t.plugin;
  }
}
class H extends S {
  constructor(t, e, {
    ...
    var h, c, p;
    super(t);
    const o = (h = e.controls) == null ? void 0 : h[s];
    this.container = (c = (o == null ? void 0 : o.container) || n.container) == null ? void 0 : c(t), this.order = (o == null ? void 0 : o.order) || n.order, this.$el = l, this.$el.style.order = (this.order || "").toString(), !r && ((p = this.container) == null || p.append(this.$el));
  }
  $(t) {
...
class it extends S {
  constructor(t, e, {
    ...
    const r = ((o = e.panels) == null ? void 0 : o[s]) || n;
    r != null && r.mount ? (h = r == null ? void 0 : r.mount(t)) == null || h.mount(this) : (this.container = (c = r == null ? void 0 : r.container) == null ? void 0 : c.call(r, t), this.onToggle = r == null ? void 0 : r.onToggle, this.onUnmount = r == null ? void 0 : r.onUnmount);
  }
  mount(t, e, s) {
...
`, us = (i, t, {
  ...
  const h = u`
    ...
    s(p, i);
      ...
      title="${o(i)}"
    >
      <div class="list-cell col-time">${V(i.time)}</div>
      <div class="list-cell col-content">${i.content}</div>
      <div class="list-cell col-date">
        ${i.date ? ut(new Date(i.date * 1e3), "yy-MM-dd HH:mm") : "-"}
      </div>
      ${e.length ? u`<div class="list-operate" title="">
            ${e(i).map(
    ([p, v]) => u`<div
  ...
  return g(h, c), c.firstElementChild;
};
class Nt extends it {
  constructor(t, e) {
  ...
  created() {
    ...
    this.list = new cs({
      ...
      createItem: (l, r) => us(l, r, {
        ...
        title: (o) => `${o.content}
${l.date ? ut(new Date(l.date * 1e3), "yyyy-MM-dd HH:mm:ss") : "-"} @ ${V(l.time, !1)}`
      }),
      overflow: 5
    });
    const s = dt(() => {
    ...
    if (e)
      if (this.focused) {
        ...
        if (n == -1 || l == -1)
          ...
          const r = n < l ? n : l, o = (n < l ? l : n) + 1;
          this.select(this.data.slice(r, o), this.focused);
        }
      } else
        this.select([t]);
    else
...
var B = /* @__PURE__ */ ((i) => (i[i.Backspace = 8] = "Backspace", i[i.Tab = 9] = "Tab", i[i.Enter = 13] = "Enter", i[i.Shift = 16] = "Shift", i[i.Ctrl = 17] = "Ctrl", i[i.Alt = 18] = "Alt", i[i.PauseBreak = 19] = "PauseBreak", i[i.CapsLock = 20] = "CapsLock", i[i.Escape = 27] = "Escape", i[i.Space = 32] = "Space", i[i.PageUp = 33] = "PageUp", i[i.PageDown = 34] = "PageDown", i[i.End = 35] = "End", i[i.Home = 36] = "Home", i[i.LeftArrow = 37] = "LeftArrow", i[i.UpArrow = 38] = "UpArrow", i[i.RightArrow = 39] = "RightArrow", i[i.DownArrow = 40] = "DownArrow", i[i.Insert = 45] = "Insert", i[i.Delete = 46] = "Delete", i[i.Zero = 48] = "Zero", i[
  ...
  i.GraveAccent = 192
  /* Tilde */
] = "GraveAccent", i[i.OpenBracket = 219] = "OpenBracket", i[i.ClosedBracket = 221] = "ClosedBracket", i[i.Quote = 222] = "Quote", i))(B || {});
const ps = () => u`
    ...
    <div class="${a}-controls-panel-wrap">
      <div class="${a}-controls-panel ${a}-controls-panel-danmaku-style">
        ...
        <div class="${a}-panel-row">
          ...
          ${window.EyeDropper ? u`<button class="${a}-danmaku-style-color-dropper mpui-button">
                拾取
              </button>` : ""}
        </div>
...
class ms {
  ...
  init(t) {
    var e, s;
    this.pickerFontsize = new U({
      container: this.$fontsizePicker,
      value: 25,
      list: (((e = t.danmakuBar) == null ? void 0 : e.fontSizeList) || $s).map(([n, l]) => ({
        value: n,
        label: l
      })),
      onChange: (n) => {
      ...
      list: [
        ...
        (n) => {
          var l;
          return (((l = t.danmakuBar) == null ? void 0 : l.modeList) || gs).indexOf(n.value) > -1;
        }
      ),
      onChange: (n) => {
        this.danmakuBar.danmakuMode = Number(n) || 1;
      }
    }), this.pickerColor = new U({
      container: this.$colorPicker,
      value: "#FFFFFF",
      list: (((s = t.danmakuBar) == null ? void 0 : s.colorList) || vs).map((n) => ({ value: n })),
      onPick: (n) => {
        this.danmakuBar.danmakuColor = Tt(n);
      },
      onChange: (n) => {
        this.$colorInput.value = n, this.$colorPreview.style.backgroundColor = n;
      },
      template: (n) => u` <div style="background-color: ${n.value}"></div> `,
...
class It extends S {
  constructor(t, e) {
    var s, n;
    super(t), this.danmakuColor = 16777215, this.danmakuMode = 1, this.danmakuFontSize = 25, this.coolDownTimer = 0, this.controller = this.plugin.controller, this.danmaku = this.plugin.danmaku, this.$wrap = this.player.$el.appendChild(
      P("div", { class: `${a}-danmakubar-wrap mpui-background` })
    ), this.loginRequired = ((s = e.danmakuBar) == null ? void 0 : s.loginRequired) || !1, g(fs, this.$wrap), this.$el = this.$wrap.querySelector(`.${a}-danmakubar`), this.$send = this.$el.querySelector(`.${a}-danmakubar-send`), this.$input = this.$el.querySelector(`.${a}-danmakubar-input`), this.$slot = this.$el.querySelector(`.${a}-danmakubar-slot`), this.$inputSlot = this.$el.querySelector(`.${a}-danmakubar-input-slot`), this.$logina = this.$el.querySelector(`.${a}-danmakubar-status-login a`), this.$logina.onclick = () => this.plugin.user.login(), this.buttonDanmakuStyle = new ms(this, this.$inputSlot, e), this.loginRequired && !e.userId && this.setLoginLimit(!0), this.player.on("video_change", () => {
      this.setLoading(!0);
    }), this.player.on("loadeddata", () => {
      this.setLoading(!1);
    }), this.player.on("login", (l) => {
      l && this.setLoginLimit(!1);
      ...
      l.keyCode == B.Enter && this.send();
    }), this.$send.onclick = () => {
      this.send();
    }, this.setPlaceHolder(((n = e.danmakuBar) == null ? void 0 : n.placeholder) || ys);
  }
  moveToWrap() {
  ...
  /** 设置登录限制 */
  setLoginLimit(t) {
    t ? this.$el.classList.add("state-login") : this.$el.classList.remove("state-login");
  }
  /** 设置加载状态 */
  setLoading(t) {
    t ? this.$el.classList.add("state-loading") : this.$el.classList.remove("state-loading");
  }
}
It.pluginName = "danmakuBar";
const ys = "发条弹幕吧~";
class bs extends S {
  constructor(t, e) {
    var s;
    if (super(t), this.padding = e.blackBorderPadding || "8px", t.plugin.buttonSettings) {
      ...
      this.checkbox = new z({
        ...
        label: "隐藏黑边"
      }), (s = t.plugin.settings) == null || s.$others.appendChild(n);
    }
    e.blackBorder && this.toggle(!0);
  }
  toggle(t) {
    t ? this.player.$content.style.setProperty("--padding", this.padding) : this.player.$content.style.setProperty("--padding", ""), this.player.emitChange("blackBorder", t);
  }
  get status() {
...
class ks {
  ...
  add(t, e) {
    t.forEach((s) => {
      const n = this.danmakuPool.findIndex((l) => s.time <= l.time);
      this.danmakuPool.splice(n === -1 ? this.danmakuPool.length : n, 0, s), s.time < this.time && (this.currentIndex += 1, e && this.draw(t));
    });
  }
  /** 弹幕池移除符合id的弹幕 */
  remove(t) {
  ...
  draw(t) {
    ...
    this.trackHeights.roll.length !== l && (this.trackHeights.roll = W(l, e)), this.trackHeights.reverse.length !== l && (this.trackHeights.reverse = W(l, e)), this.trackHeights.top.length !== l && (this.trackHeights.top = W(l, e)), this.trackHeights.bottom.length !== l && (this.trackHeights.bottom = W(l, e));
    const r = (m) => {
    ...
    }, h = (m) => this.baseSpeed * (1 + this.deltaSpeed * m) * this.speed, c = (m, $) => [
      ...this.container.querySelectorAll(`.${this.classPrefix}-danmaku-${m}`)
    ].filter((d) => d.dataset.track === `${$}`), p = (m, $, d) => {
      t:
        for (let f = 0; this.overlap || f < l; f++) {
          const L = c($, f);
          let k = this.danmakuTracks[$][f];
          if (this.danmakuTracks[$][f] = L, k && k.length) {
            if ($ === "roll") {
              const A = s / h(d);
              k.length !== L.length && (k = L);
              for (const w of k) {
                const x = r(w) - 10;
                if (this.trackHeights[$][f] = parseInt(w.style.fontSize) + this.trackPadding, x <= s - A * h(w.getBoundingClientRect().width) || x <= 0)
                  continue t;
              }
            } else if ($ === "reverse") {
              const A = s / h(d);
              ...
              continue t;
            return this.danmakuTracks[$][f].push(m), m.addEventListener("animationend", () => {
              var w, x;
              const A = (w = this.danmakuTracks[$][f]) == null ? void 0 : w.indexOf(m);
              A && ((x = this.danmakuTracks[$][f]) == null || x.splice(A, 1));
            }), f;
          } else
            return Array.isArray(this.danmakuTracks[$][f]) ? this.danmakuTracks[$][f].push(m) : this.danmakuTracks[$][f] = [m], m.addEventListener("animationend", () => {
              var w, x;
              const A = (w = this.danmakuTracks[$][f]) == null ? void 0 : w.indexOf(m);
              A && ((x = this.danmakuTracks[$][f]) == null || x.splice(A, 1));
            }), f;
    ...
    for (let m = 0; m < t.length; m++) {
      ...
      d.classList.add(`${this.classPrefix}-danmaku-item`), d.classList.add(`${this.classPrefix}-danmaku-${tt[$.mode]}`), d.innerHTML = `${$.content.replace(/(\\n)/g, `
`)}`, typeof $.color == "number" ? d.style.color = At($.color) : d.style.color = $.color, d.style.opacity = this.opacity.toString(), d.style.fontSize = +$.size * this.fontScale + "px", $.fromHere && (d.style.border = "2px solid white"), d.addEventListener("animationend", () => {
        [...this.container.children].indexOf(d) > -1 && this.container.removeChild(d);
      });
      const f = this.measureTextWidth(
        $.content,
      ...
      switch (L) {
        case "roll":
        case "reverse":
          if (A = p(d, L, f), k = A % l, k >= 0) {
            const w = this.trackHeights[L].slice(0, l).reduce((T, q) => T + q, 0), x = this.trackHeights[L].slice(0, k).reduce((T, q) => T + q, 0) % w;
            if (x + parseInt(d.style.fontSize) + this.trackPadding > n) {
              (y = this.danmakuTracks[L][m]) == null || y.pop();
              return;
            }
            const D = h(f), b = f + s + this.startDistance * 2;
            d.dataset.track = A.toString(), d.style.width = f + 1 + "px", d.style.top = x + "px", d.style.fontFamily = this.fontFamily, d.style.fontWeight = this.fontWeight, d.style.setProperty("--duration", `${b / D}s`), d.style.setProperty("--offset", `${s + this.startDistance}px`), d.style.setProperty("--translateX", `${-b}px`);
          }
          break;
        case "top":
          if (k = p(d, L, 0) % l, k >= 0) {
            ...
            for (const b of x)
              w.push(...b);
            const D = w.map((b) => parseInt(b.style.fontSize) + this.trackPadding).slice(0, k).reduce((b, T) => b + T, 0);
            if (D + parseInt(d.style.fontSize) + this.trackPadding > n) {
              (E = this.danmakuTracks[L][m]) == null || E.pop();
              return;
            }
            d.dataset.track = k.toString(), d.style.width = f + 1 + "px", d.style.marginLeft = `-${(f + 1) * 0.5}px`, d.style.top = D + "px", d.style.fontFamily = this.fontFamily, d.style.fontWeight = this.fontWeight, d.style.setProperty("--duration", `${this.baseDuration / this.speed}s`);
          }
          break;
        case "bottom":
          if (L = "bottom", k = p(d, L, 0) % l, k >= 0) {
            const w = [], x = this.danmakuTracks.bottom;
            for (const b of x)
              w.push(...b);
            const D = w.map((b) => parseInt(b.style.fontSize) + this.trackPadding).slice(0, k).reduce((b, T) => b + T, 0);
            if (D + parseInt(d.style.fontSize) + this.trackPadding > n) {
              (_ = this.danmakuTracks[L][m]) == null || _.pop();
              return;
            }
            d.dataset.track = k.toString(), d.style.width = f + 1 + "px", d.style.marginLeft = `-${(f + 1) * 0.5}px`, d.style.bottom = D + "px", d.style.fontFamily = this.fontFamily, d.style.fontWeight = this.fontWeight, d.style.setProperty("--duration", `${this.baseDuration / this.speed}s`);
          }
          break;
        default:
  ...
  captureDanmakuDOM(t, e, s, n = !1) {
    ...
    for (const o of r)
      if (o.innerHTML) {
        const h = o.getBoundingClientRect(), c = this.container.getBoundingClientRect(), p = h.left - c.left, v = h.right - c.left, y = h.top - c.top, E = h.bottom - c.top;
        if (t >= p - s && t <= v + s && e >= y - s && e <= E + s && (l.push(o), n))
          return l;
      }
    return l;
  }
...
function ws() {
  const i = [], t = xt.length;
  for (let e = 0; e < 8; e++)
    i.push(xt[Math.floor(Math.random() * t)]);
  return wt++, i.join("") + `${wt}`;
}
class xs {
  constructor({ defaultParser: t }) {
    this.defaultParser = t || "bilibili-xml", this.list = {
      ...
      "bilibili-xml": (e) => ((n) => {
        const l = [], o = new DOMParser().parseFromString(n, "text/xml").childNodes, h = (c) => {
          var p;
          for (let v = 0; v < c.length; v++) {
            const y = c[v];
            if ((p = y == null ? void 0 : y.attributes) != null && p.length && v > 0) {
              const E = y.attributes[0].nodeValue.split(","), _ = y.innerHTML;
              l.push([_, E]);
            } else
              y.childNodes.length > 0 && h(y.childNodes);
          }
        };
        return h(o), l.map(([c, p]) => ({
          time: +p[0],
          mode: +p[1],
          color: +p[3],
          user: p[6],
...
class Ss {
  ...
  async send(t) {
    var e, s;
    if (!((s = (e = this.danmaku) == null ? void 0 : e.api) != null && s.send))
      throw "发送失败";
    return await this.danmaku.api.send(t, this.player.videoInfo.danmakuId || 0).then((n) => (this.danmaku.add(
      [
        Object.assign(
          {
            id: `send:${Date.now()}`,
  ...
  async report(t) {
    var e, s;
    if (!((s = (e = this.danmaku) == null ? void 0 : e.api) != null && s.report))
      throw "操作失败";
    return await this.danmaku.api.report(t).then((n) => (this.danmaku.remove([t.id]), n)).catch((n) => {
      throw n;
    });
  }
  /**
  ...
  async delete(t) {
    var e, s;
    if (!((s = (e = this.danmaku) == null ? void 0 : e.api) != null && s.delete))
      throw "操作失败";
    return await this.danmaku.api.delete(t).then((n) => (this.danmaku.remove(t.map((l) => l.id)), n)).catch((n) => {
      throw n;
    });
  }
  /**
  ...
  async recall(t) {
    var e, s;
    if (!((s = (e = this.danmaku) == null ? void 0 : e.api) != null && s.recall))
      throw "操作失败";
    return await this.danmaku.api.recall(t).then((n) => (this.danmaku.remove([t.id]), n)).catch((n) => {
      throw n;
    });
  }
  /**
  ...
  async blockUser(t, e) {
    var s, n;
    if (!((n = (s = this.danmaku) == null ? void 0 : s.api) != null && n.blockUser))
      throw "操作失败";
    return await this.danmaku.api.blockUser(t, e).then(() => {
  ...
  async blockContent(t, e) {
    var s, n;
    if (!((n = (s = this.danmaku) == null ? void 0 : s.api) != null && n.blockContent))
      throw "操作失败";
    return await this.danmaku.api.blockContent(t, e).then((l) => {
...
class Bt extends S {
  constructor(t, e) {
    var s;
    super(t), this.lastDanmakuId = 0, this.list = [], this.handler = {}, this.format = ((s = e.danmaku) == null ? void 0 : s.format) || "mfuns", this.parser = new xs({ defaultParser: this.format }), this.operate = new Ss(this.player, this), this.api = e.danmakuApi || {}, this.allowNewDanmaku = !1, this.$el = P("div", { class: `${a}-danmaku-wrap` }), this.$rowContainer = this.$el.appendChild(
      P("div", { class: `${a}-row-danmaku-container` })
    ), this.player.$content.after(this.$el), this.engine = new ks(this.$rowContainer, {
      fontScale: 1,
      fontFamily: "SimHei",
      fontWeight: "bold",
      speed: 1,
  ...
  load(t) {
    var e, s;
    (s = (e = this.api).get) == null || s.call(e, { id: t }).then((n) => {
  ...
  loadNew(t) {
    ...
    this.player.emit("danmaku:new_loading", e), (n = (s = this.api).get) == null || n.call(s, { id: t, offset: this.lastDanmakuId }).then((l) => {
  ...
  add(t, e) {
    ...
    const s = [], n = {};
    t.forEach((o) => {
      o.mode < 7 ? s.push(o) : n[o.mode] ? n[o.mode].push(o) : n[o.mode] = [o];
    }), s.length && this.engine.add(s, e);
    for (const o in n)
      (r = (l = this.handler)[o]) == null || r.call(l, n[o]);
  }
  /**
   * 根据id从弹幕池中移除弹幕
   * @param ids 要移除的弹幕id
...
Bt.pluginName = "danmaku";
class Rt extends S {
  constructor(t, e) {
    super(t), e.widescreen && this.enter(), this._handler = e.widescreenToggle;
  }
  enter() {
    var t;
    this.status || (this.player.$el.classList.add("state-widescreen"), (t = this._handler) == null || t.call(this, !0), this.player.emit("widescreen:enter"));
  }
  exit() {
    var t;
    this.status && (this.player.$el.classList.remove("state-widescreen"), (t = this._handler) == null || t.call(this, !1), this.player.emit("widescreen:exit"));
  }
  get status() {
    return this.player.$el.classList.contains("state-widescreen");
  }
}
Rt.pluginName = "widescreen";
const Ls = u`
...
Vt.pluginName = "buttonPlay";
const _s = u`
  <div
    class="${a}-controls-button ${a}-button_prev state-autohide state-disabled"
  >
    <div class="${a}-controls-button-icon">
  ...
  constructor(t, e) {
    ...
    g(_s, s), super(t, e, {
      ...
      defaultOptions: {
        container: (l) => {
          var r;
          return (r = l.plugin.controller) == null ? void 0 : r.$left;
        },
        order: 0
      },
      el: s.querySelector(`.${a}-controls-button`)
    }), this.$icon = this.$(`.${a}-controls-button-icon`), this.$tooltip = this.$(".mpui-tooltip"), this.singleHide = ((n = e.switchButton) == null ? void 0 : n.singleHide) ?? !0;
  }
  created() {
    this.$icon.addEventListener("click", () => {
      this.player.prev();
    }), this.player.on("video_change", () => {
      this.player.hook.call("hasPrev", void 0, !1).then((t) => {
        this.setDisabled(!t), this.singleHide && !t ? this.player.hook.call("hasNext", void 0, !1).then((e) => {
          this.$el.classList.toggle("state-hidden", !e);
        }) : this.$el.classList.remove("state-hidden");
      });
    });
  }
  setDisabled(t) {
    this.$el.classList.toggle("state-disabled", t);
  }
  /** 自动隐藏上一个/下一个按钮 */
  setAutoHide(t) {
    this.$el.classList.toggle("state-autohide", t);
  }
  get disabled() {
    return this.$el.classList.contains("state-disabled");
  }
}
Ot.pluginName = "buttonPrev";
const Es = u`
  <div
    class="${a}-controls-button ${a}-button_next state-autohide state-disabled"
  >
    <div class="${a}-controls-button-icon">
...
class zt extends H {
  ...
  created() {
    this.$icon.addEventListener("click", () => {
      ...
      this.player.hook.call("hasNext", void 0, !1).then((t) => {
        this.setDisabled(!t), this.singleHide && !t ? this.player.hook.call("hasPrev", void 0, !1).then((e) => {
          this.$el.classList.toggle("state-hidden", !e);
        }) : this.$el.classList.remove("state-hidden");
      });
    });
...
class Wt extends H {
  ...
  created() {
    this.player.on("timeupdate", (t) => {
      ...
      this.$el.classList.add("state-input"), this.$input.value = V(this.player.time), this.valueBeforeEdited = this.$input.value, this.$input.focus();
...
class Kt extends H {
  ...
  created() {
    this.player.on("pip:enter", () => {
      ...
      const { pip: t } = this.plugin;
      t && (t.status ? t.exit() : t.enter());
    });
  }
}
Kt.pluginName = "buttonPip";
const qs = u`
  <div class="${a}-controls-button ${a}-button_widescreen">
    <div class="${a}-controls-button-icon">
...
class Qt extends H {
  ...
  created() {
    this.player.on("widescreen:enter", () => {
      this.$el.classList.add("state-on"), this.$tooltip.innerText = "退出宽屏模式";
    }), this.player.on("widescreen:exit", () => {
      this.$el.classList.remove("state-on"), this.$tooltip.innerText = "宽屏模式";
    }), this.$icon.addEventListener("click", () => {
      const { widescreen: t } = this.plugin;
      t && (t.status ? t.exit() : t.enter());
    });
  }
}
Qt.pluginName = "buttonWidescreen";
const Ms = u`
  <div class="${a}-controls-button ${a}-button_webscreen">
    <div class="${a}-controls-button-icon">
...
class Zt extends H {
  ...
  created() {
    this.player.on("webscreen:enter", () => {
      ...
      const { webscreen: t } = this.plugin;
      t && (t.status ? t.exit() : t.enter());
    });
  }
}
Zt.pluginName = "buttonWebscreen";
const Ns = u`
...
class Gt extends H {
  ...
  created() {
    this.player.on("fullscreen:enter", () => {
      ...
      const { fullscreen: t } = this.plugin;
      t && (t.status ? t.exit() : t.enter());
    });
  }
}
Gt.pluginName = "buttonFullscreen";
const Is = u`
...
Jt.pluginName = "buttonDanmakuToggle";
const Bs = u`
...
class Ct extends H {
  ...
  created() {
    ...
    }), this.sliderArea.drag(25), this.sliderSize = new K({
...
Ct.pluginName = "buttonDanmakuSettings";
const Rs = () => u`
  <div class="${a}-about">
    <div class="${a}-about-logo"></div>
    <div class="${a}-about-version">version ${M.version}-${M.gitHash}</div>
    <div>github：<a href="${Oe}" target="_blank">mfuns-cn/mfunsPlayer</a></div>
    <div>开发者：</div>
...
class se extends S {
    ...
    const e = [
      {
        ...
        onClick: () => {
          var s;
          (s = this.plugin.hotkeys) == null || s.toggle(!0);
        }
      },
      {
        name: `Mfuns Player v${M.version}-${M.gitHash}`,
        onClick: () => {
          var s;
          (s = this.plugin.about) == null || s.toggle(!0);
        }
      }
    ];
    this.container = P("div", { class: `${a}-contextmenu-wrap` }), g(Os(e), this.container), this.$el = this.container.querySelector(`.${a}-contextmenu`), this.$danmaku = this.$el.querySelector(`.${a}-contextmenu-danmaku`), this.$menu = this.$el.querySelector(`.${a}-contextmenu-menu`), this.player.$main.appendChild(this.container);
  }
  created() {
  ...
  showDanmaku(t) {
    ...
    t != null && t.length ? this.$danmaku.style.display = "" : this.$danmaku.style.display = "none", g(
      zs(
        t,
        (r) => {
          const o = this.player.userId && r.user == this.player.userId;
          return [
            [
              "举报",
              (h) => {
              ...
              !o && (e == null ? void 0 : e.report)
            ],
            [
              "屏蔽",
              (h) => {
                s == null || s.blockUser(h.user, !0);
              },
              !o && (e == null ? void 0 : e.blockUser)
            ],
            [
              "撤回",
              (h) => {
...
class ne {
  ...
  initKey() {
    document.addEventListener("keydown", (t) => {
      var n, l;
      const e = (n = document.activeElement) == null ? void 0 : n.tagName.toUpperCase(), s = (l = document.activeElement) == null ? void 0 : l.getAttribute("contenteditable");
...
class le {
  constructor(t, e) {
    ...
    const s = (l) => {
      var o;
      const { clientX: r } = l;
      l.preventDefault(), l.stopPropagation(), this.distance = r - this.nLeft, this.setPlayed(this.nValue), this.updateTip(), (o = window.getSelection()) == null || o.removeAllRanges();
    }, n = (l) => {
      var r;
...
class ce extends S {
  constructor(t) {
    super(t), this.player.on("enterpictureinpicture", () => {
      this.player.$el.classList.add("state-pip"), this.player.emit("pip:enter");
    }), this.player.on("leavepictureinpicture", () => {
      this.player.$el.classList.remove("state-pip"), this.player.emit("pip:exit");
    }), this.player.on("pip:enter", () => {
      var e;
      (e = this.plugin.fullscreen) == null || e.exit();
    }), this.player.on("fullscreen:enter", () => {
      this.exit();
    });
  }
...
class de {
  constructor(t) {
    ...
    const e = () => {
      this.status ? (this.player.$el.classList.add("state-fullscreen"), this.player.emit("fullscreen:enter")) : (this.player.$el.classList.remove("state-fullscreen"), this.player.emit("fullscreen:exit"));
    };
...
de.pluginName = "fullscreen";
class ue extends S {
  created(t) {
    this.apiLogin = t.login, this.player.on("options_set", ({ userId: e }) => {
      e != null && (this.player.userId = e, this.player.emit("login", e));
    });
  }
  /** 调用 */
  async login() {
    var t;
    await ((t = this.apiLogin) == null ? void 0 : t.call(this).then((e) => {
      e && this.player.emit("options_set", e);
    }));
  }
}
ue.pluginName = "user";
const Qs = [oe, ce, de], Zs = [ae, he, ie, re, ne, se, ue], Gs = [
  le,
  Vt,
  Ot,
  zt,
...
me.pluginName = "autoPlay";
class $e extends S {
  constructor(t, e) {
    super(t), this._status = !1, e.seekLast && (this.toggle(!0), this.player.once("loadedmetadata", () => {
  ...
  toggle(t) {
    t ? this._status = !0 : this._status = !1, this.player.emitChange("seekLast", t);
  }
  get status() {
    return this._status;
  }
}
$e.pluginName = "seekLast";
const ot = {
...
class ge extends S {
  constructor(t, e) {
    super(t), this.properties = {}, this._matchDarkScheme = window.matchMedia("(prefers-color-scheme: dark)"), this.themeElement = [this.player.container], this.setTheme(e.theme || {}), this._handleDarkScheme = (s) => {
      this.player.$el.classList.toggle("mpui-dark", s.matches);
    };
  }
  /** 设置主题 */
  setTheme(t) {
  ...
  created() {
    this.player.hook.register(
      ...
      if (this.list && this.part > 1)
        return this.set(this.part - 1), !1;
    }), this.player.hook.register("hasNext", () => this.list && this.part < this.num || void 0), this.player.hook.register("hasPrev", () => this.list && this.part > 1 || void 0);
  }
  /** 设置分P */
  set(t, e) {
    var n;
...
class fe extends S {
  ...
  load(t) {
    this.player.emit("video_load", t);
    const { url: e, type: s, play: n, time: l } = t, r = this.player.$video, o = r.cloneNode();
    o.src = e, o.addEventListener(
      "loadedmetadata",
      () => {
        this.player.$content.insertBefore(o, r), o.currentTime = this.player.time, !this.player.paused && o.play(), o.addEventListener(
          "canplay",
          () => {
            var h;
            this.player.emit("video_load", t), (h = this.plugin.pip) != null && h.status && o.requestPictureInPicture(), this.player.bindVideo(o), r.remove(), n == !0 && this.player.paused && this.player.play(), n == !1 && !this.player.paused && this.player.pause(), l != null && this.player.seek(l);
          },
          { once: !0 }
        );
      },
...
be.pluginName = "buttonDanmakuList";
class ke extends S {
  constructor(t, e) {
    super(t), this.value = "";
  }
...
class we extends H {
  ...
  created() {
    this.player.on("video_change", (t) => {
      ...
      e != null && e.quality ? this._select(e == null ? void 0 : e.quality, e.label) : this.player.once("loadedmetadata", () => {
  ...
  _update(t) {
    if (t && t == this._list)
      return;
    const e = [];
    t == null || t.forEach(({ quality: s, label: n }) => {
      s && !e.find((l) => l.quality == s) && e.push({ quality: s, label: n });
    }), e.length ? (console.log("mmmmmmmm"), g(
      St(e, (s) => {
        var n;
        (n = this.plugin.quality) == null || n.set(s);
      }),
      this.$list
    )) : this.player.once("loadedmetadata", () => {
      const { videoWidth: s, videoHeight: n } = this.player.$video;
      g(
        St([{ quality: `${Math.min(s, n)}P` }], (l) => {
          var r;
          (r = this.plugin.quality) == null || r.set(l);
        }),
        this.$list
      );
    }), this._list = t;
...
class xe extends S {
  constructor(t, e) {
    super(t), this.$el = P("div", { class: `${a}-videostatus` }), g(ii, this.$el), this.$paused = this.$el.querySelector(`.${a}-videostatus-paused`), this.$loading = this.$el.querySelector(`.${a}-videostatus-loading`), this.$volume = this.$el.querySelector(`.${a}-videostatus-volume`), this.player.$area.appendChild(this.$el);
  }
}
xe.pluginName = "videoStatus";
const ni = u`
...
class Se extends S {
  constructor(t, e) {
    var s, n;
    super(t), this.$el = P("div", { class: `${a}-loadingmask` }), g(ni, this.$el), this.$info = this.$el.querySelector(`.${a}-loadingmask-info`), this.$tips = this.$el.querySelector(`.${a}-loadingmask-tips`), this.getTips = (s = e.loadingMask) == null ? void 0 : s.getTips, this.delay = ((n = e.loadingMask) == null ? void 0 : n.delay) || 0, this.player.$main.appendChild(this.$el);
  }
  created() {
  ...
  _add(t, e) {
    const s = P("div", { class: `${a}-loadingmask-info-item` });
  ...
  constructor(t, e) {
    super(t, e, {
      ...
      defaultOptions: {
        container: (s) => {
          var n;
          return (n = s.plugin.header) == null ? void 0 : n.$left;
        },
        order: 0
      },
      el: P("div", { class: `${a}-videotitle` })
    });
  }
  created() {
    this.player.on("video_change", (t) => {
...
_e.pluginName = "header";
class Ee extends S {
  constructor(t, e) {
    var s;
    if (super(t), this._shown = !1, this._dragging = !1, this._enabled = !1, this.$el = P("div", { class: `${a}-mini` }), this.player.$el.appendChild(this.$el), this.player.on("intersection", (n) => {
      var r;
      const l = !this._enabled || ((r = this.plugin.pip) == null ? void 0 : r.status);
      n ? !l && this._hide() : !l && this._show(), console.log(n);
      ...
      this.checkbox = new z({
        ...
        label: "小窗模式"
      }), (s = t.plugin.settings) == null || s.$others.appendChild(n), t.watch("mini", (l) => {
        var r;
        return (r = this.checkbox) == null ? void 0 : r.setValue(l);
      });
    }
    e.blackBorder && this.toggle(!0);
  }
  created() {
    this.$el.addEventListener("click", () => {
      ...
      const { clientX: e, clientY: s } = t, { offsetLeft: n, offsetTop: l } = this.$el;
      console.log([n, l]);
      const r = e - n, o = s - l, h = (p) => {
        this._dragging = !0;
        const { clientX: v, clientY: y } = p;
        this.$el.style.left = `${v - r}px`, this.$el.style.top = `${y - o}px`;
      }, c = () => {
    ...
    }), this.player.on("pip:enter", () => this._hide()), this.player.on("pip:exit", () => !this.player.intersecting && this._show());
  }
  pluginsReady(t) {
    t.mini && this.toggle(!0);
  }
  _show() {
    this._shown || (this.player.$area.style.height = `${this.player.$area.clientHeight}px`, this.$el.append(...this.player.$area.children), this.$el.classList.add("state-show"), this._shown = !0);
  }
  _hide() {
    this._shown && (this.player.$area.style.height = "", this.player.$area.append(...this.$el.children), this.$el.classList.remove("state-show"), this._shown = !1);
  }
  /** 播放器小窗启用状态 */
  get enabled() {
  ...
  toggle(t) {
    if (t) {
      if (this.enabled)
        return;
      !this.player.intersecting && this._show(), this.player.emitChange("mini", !0);
    } else {
      if (!this.enabled)
        return;
      this._hide(), this.player.emitChange("mini", !1);
    }
    this._enabled = t;
  }
}
...
class li extends S {
  constructor(t, e) {
    var s;
    if (super(t), this.status = !1, this.$mask = P("div", { class: `${a}-lightoff-mask` }), t.plugin.buttonSettings) {
      const n = document.createElement("div");
      this.checkbox = new z({
        container: n,
        value: this.status,
        ...
        label: "关灯模式"
      }), (s = t.plugin.settings) == null || s.$others.appendChild(n), t.watch("lightOff", (l) => {
        var r;
        return (r = this.checkbox) == null ? void 0 : r.setValue(l);
      });
    }
  ...
  toggle(t) {
    t ? this.player.$el.classList.add("state-lightoff") : this.player.$el.classList.remove("state-lightoff"), this.player.emitChange("lightOff", t);
  }
}
class Te extends S {
  constructor(t) {
    super(t), this.player.on("webscreen:enter", () => {
      var e;
      (e = this.plugin.fullscreen) == null || e.exit();
    }), this.player.on("fullscreen:enter", () => {
      this.exit();
  ...
  /** 进入网页全屏 */
  enter() {
    this.status || (this.player.$el.classList.add("state-webscreen"), this.player.emit("webscreen:enter"));
  }
  /** 退出网页全屏 */
  exit() {
    this.status && (this.player.$el.classList.remove("state-webscreen"), this.player.emit("webscreen:exit"));
  }
  /** 是否处于网页全屏模式 */
  get status() {
